...
bool has_grouped_manual_pattern(const MixedFilamentManager* mixed_mgr, size_t num_physical, unsigned int filament_id_1based)
{
    if (!(mixed_mgr && mixed_mgr->is_mixed(filament_id_1based, num_physical)))
        return false;
    const MixedFilament* mixed_row = mixed_mgr->mixed_filament_from_id(filament_id_1based, num_physical);
...
{
    return use_base_infill_filament(layer_tools, region) ? region.config().wall_filament.value :
...
unsigned int grouped_manual_pattern_mixed_filament_id_for_layer(const LayerTools& layer_tools, unsigned int configured_filament_id_1based)
{
...
{
    const unsigned int grouped_id = grouped_manual_pattern_mixed_filament_id_for_layer(layer_tools, configured_filament_id_1based);
    if (grouped_id == 0)
        return 0;

    const int innermost_perimeter_index = std::max(0, region.config().wall_loops.value - 1);
    return layer_tools.mixed_mgr->resolve_perimeter(grouped_id, layer_tools.num_physical, layer_tools.layer_index,
                                                    innermost_perimeter_index, float(layer_tools.print_z), float(layer_tools.layer_height));
}
...
{
    return resolve_mixed_with_layer_heights(mixed_mgr, num_physical, filament_id, this->layer_index, float(this->print_z),
                                            float(this->layer_height), mixed_layer_height_a, mixed_layer_height_b, mixed_base_layer_height);
}
...
void ToolOrdering::update_mixed_layer_height_settings()
{
    ...
    if (m_print_full_config != nullptr && m_print_full_config->has("mixed_color_layer_height_a") &&
                                     ...
                                     const std::vector<std::pair<double, unsigned int>>& per_layer_extruder_switches)
{
    ...
    for (auto support_layer : object.support_layers()) {
        ...
        unsigned int  extruder_support   = resolve_mixed(object.config().support_filament.value, layer_tools.layer_index,
                                                         float(support_layer->print_z), float(support_layer->height));
...
void                     ToolOrdering::assign_custom_gcodes(const Print& print)
{
    ...
    assert(print.config().print_sequence == PrintSequence::ByLayer || print.uses_layered_print_sequence_for_overlapping_extruders());
    ...
    for (auto it_lt = m_layer_tools.rbegin(); it_lt != m_layer_tools.rend(); ++it_lt) {
        ...
        if (custom_gcode.print_z > 0.5 * (print_z_below + lt.print_z)) {
            ...
            if (!ignore_tool_and_color_changes) {
                // If it is color change, it will actually be useful as the exturder above will print.
                // BBS
                apply_color_change = color_change ? mode == CustomGCode::SingleExtruder ||
...
unsigned int ToolOrdering::resolve_mixed(unsigned int filament_id_1based, int layer_index, float layer_print_z, float layer_height) const
{
    return resolve_mixed_with_layer_heights(m_mixed_mgr, m_num_physical, filament_id_1based, layer_index, layer_print_z, layer_height,
                                            m_mixed_layer_height_a, m_mixed_layer_height_b, m_mixed_base_layer_height);
}
