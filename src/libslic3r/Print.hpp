...
template<typename T> class ConstVectorOfPtrsAdaptor
{
public:
...
public:
    ...
    // Transformation of this ModelObject into one of the associated PrintObjects (all PrintObjects derived from a single modelObject differ
    // by a Z rotation only). This transformation is used to calculate VolumeExtents.
...
public:
    ...
    static void remove_bridges_from_contacts(const Layer* lower_layer,
    ...
    // Height is used for slicing, for sorting the objects by height for sequential printing and for checking vertical clearence in
    // sequential print mode. The height is snug.
    ...
    // BBS
    const ExtrusionEntityCollection& object_skirt() const { return m_skirt; }
    ...
    SupportLayerPtrs::iterator insert_support_layer(
        SupportLayerPtrs::iterator pos, size_t id, size_t interface_id, coordf_t height, coordf_t print_z, coordf_t slice_z);

    // Initialize the layer_height_profile from the model_object's layer_height_profile, from model_object's layer height table, or from
    // slicing parameters. Returns true, if the layer_height_profile was changed.
    ...
    static SlicingParameters slicing_parameters(const DynamicPrintConfig& full_config,
    ...
    void project_and_append_custom_facets(bool                                  seam,
                                          EnforcerBlockerType                   type,
                                          std::vector<Polygons>&                expolys,
                                          std::vector<std::pair<Vec3f, Vec3f>>* vertical_points = nullptr) const;
...
struct FakeWipeTower
{
    ...
    void set_fake_extrusion_data(const Vec2f&                                p,
                                 float                                       w,
                                 float                                       h,
                                 float                                       lh,
                                 float                                       d,
    ...
    std::vector<ExtrusionPaths> getFakeExtrusionPathsFromWipeTower() const
    {
        ...
        for (float h = 0.f; h < height; h += layer_height) {
            ...
            if (h == 0.f) { // add brim
                ...
                fakeBrim.polyline          = {wtbminCorner,
                                              {wtbmaxCorner.x(), wtbminCorner.y()},
                                              wtbmaxCorner,
                                              {wtbminCorner.x(), wtbmaxCorner.y()},
                                              wtbminCorner};
            ...
            if (cone_base_R > 0.) {
                ...
                for (double alpha = 0; alpha < 2.01 * M_PI; alpha += 2 * M_PI / 20.)
                    path.polyline.points.emplace_back(
                        Point::new_scale(width / 2. + r * std::cos(alpha) / cone_scale_x, depth / 2. + r * std::sin(alpha)));
                paths.back().emplace_back(path);
                if (hh == 0.f) { // Cone brim.
                    for (float bw = brim_width; bw > 0.f; bw -= 3.f) {
                        ...
                        for (double alpha = 0; alpha < 2.01 * M_PI;
                            ...
                            path.polyline.points.emplace_back(
                                Point::new_scale(width / 2. +
                                                     cone_base_R * std::cos(alpha) / cone_scale_x * (1. + cone_scale_x * bw / cone_base_R),
...
enum FilamentTempType { HighTemp = 0, LowTemp, HighLowCompatible, Undefine };
...
public:
    ...
    std::string export_gcode(const std::string&          path_template,
    ...
    bool                      uses_layered_print_sequence_for_overlapping_extruders() const;
    ...
    std::map<ObjectID, ExtrusionEntityCollection>& get_brimMap() { return m_brimMap; }
    ...
    std::vector<size_t>              layers_sorted_for_object(float                     start,
                                                              float                     end,
    ...
    void export_gcode_from_previous_file(const std::string&          file,
                                         GCodeProcessorResult*       result,
                                         ThumbnailsGeneratorCallback thumbnail_cb = nullptr);
    ...
    std::string get_conflict_string() const
    {
        ...
        if (m_conflict_result) {
            result = "Found gcode path conflicts between object " + m_conflict_result.value()._objName1 + " and " +
                     m_conflict_result.value()._objName2;
        }
    ...
    // BBS
    static StringObjectException sequential_print_clearance_valid(const Print&                            print,
    ...
    {
        return std::all_of(this->objects().begin(), this->objects().end(),
                           [&](PrintObject* obj) { return obj->height() < scale_(this->config().nozzle_height.value); });
    }
