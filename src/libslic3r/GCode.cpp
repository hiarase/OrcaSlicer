...
std::string WipeTowerIntegration::append_tcr(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_extruder_id, double z) const
{
    if (new_extruder_id != -1 && new_extruder_id != tcr.new_tool)
        throw Slic3r::InvalidArgument(Slic3r::format("Error: WipeTowerIntegration::append_tcr unexpected toolchange: layer_idx=%1% "
                                                     "tool_change_idx=%2% requested=%3% expected=%4% initial=%5%",
...
std::string WipeTowerIntegration::tool_change(
    GCode& gcodegen, int extruder_id, bool finish_layer, bool local_z_unplanned, double local_z_nominal_layer_z)
{
    std::string gcode;
    auto        realign_nominal_toolchange_idx = [&](int requested_extruder_id) {
        ...
        if (toolchanges.empty())
            return;
        if (size_t(m_tool_change_idx) < toolchanges.size() && toolchanges[size_t(m_tool_change_idx)].new_tool == requested_extruder_id) {
            return;
        }
        ...
        BOOST_LOG_TRIVIAL(warning) << "Wipe tower nominal toolchange state mismatch, realigning"
                                   ...
                                   << " requested=" << requested_extruder_id << " matched_idx=" << matched_idx
    ...
    auto emit_local_z_unplanned_toolchange = [&]() -> std::string {
        ...
        if (m_layer_idx >= 0 && size_t(m_layer_idx) < m_local_z_tool_changes.size() &&
            ...
            if (local_z_tool_change_idx < layer_local_z_tool_changes.size()) {
                ...
                if (local_z_tcr.new_tool == extruder_id && (current_tool < 0 || local_z_tcr.initial_tool == current_tool)) {
                ...
                BOOST_LOG_TRIVIAL(warning) << "Local-Z preplanned wipe tower sequence mismatch, falling back"
                                           << " layer_idx=" << m_layer_idx << " requested_extruder=" << extruder_id
        ...
        if (slot_idx >= layer_slots.size() || layer_slots.empty()) {
            BOOST_LOG_TRIVIAL(warning) << "Local-Z toolchange reserve exhausted"
                                       << " layer_idx=" << m_layer_idx << " extruder_id=" << extruder_id
                                       << " reserved_slots=" << layer_slots.size() << " consumed_slots=" << slot_idx;
            return gcodegen.set_extruder(unsigned(extruder_id), toolchange_print_z);
        }
        ...
        if (gcodegen.m_curr_print != nullptr && gcodegen.writer().extruder() != nullptr) {
            ...
            WipeTower2 local_z_wipe_tower(print_config, print.default_region_config(), print.get_plate_index(), print.get_plate_origin(),
                                          wipe_volumes, current_tool);
            ...
            WipeTower::ToolChangeResult local_z_tcr = local_z_wipe_tower.local_z_tool_change(size_t(extruder_id), slot,
                                                                                             float(print_config.prime_volume));
        ...
        const float flow_per_mm     = filament_area > 0.f ?
                                          (layer_height * (line_width - layer_height * float(1.f - M_PI / 4.f)) / filament_area) *
                                              std::max(0.f, extra_flow) :
                                          0.f;
        ...
        auto transform_wt_pt = [&alpha, this](const Vec2f& pt) -> Vec2f { return Eigen::Rotation2Df(alpha) * pt + m_wipe_tower_pos; };
        ...
        if (!is_approx(tower_z, current_z)) {
            ...
            const bool      can_wipe        = active_extruder != nullptr && gcodegen.config().wipe.get_at(active_extruder->id()) &&
        ...
        BOOST_LOG_TRIVIAL(debug) << "Local-Z toolchange emitted on wipe tower reserve"
                                 << " layer_idx=" << m_layer_idx << " extruder_id=" << extruder_id << " reserve_slot=" << (slot_idx - 1);
        return gcode;
    };
...
static unsigned int gcode_primary_object_extruder(const PrintObject &object)
{
...
static bool gcode_have_disjoint_object_extruders(const PrintObject &lhs, const PrintObject &rhs)
{
...
static Polygon gcode_print_instance_convex_hull(const PrintObject &object, const PrintInstance &instance)
{
...
static bool gcode_allows_alternating_overlap_initial_gap(const PrintObject &object)
{
    const Print *print = object.print();
    if (print == nullptr)
        return false;

    const unsigned int object_extruder = gcode_primary_object_extruder(object);
    if (object_extruder == static_cast<unsigned int>(-1))
        return false;

    for (const PrintObject *other : print->objects()) {
        if (other == nullptr || other == &object || !gcode_have_disjoint_object_extruders(object, *other))
            continue;

        const unsigned int other_extruder = gcode_primary_object_extruder(*other);
        if (other_extruder == static_cast<unsigned int>(-1) || other_extruder == object_extruder)
            continue;

        if (other->layers().empty() || !other->layers().front()->has_extrusions())
            continue;

        for (const PrintInstance &object_instance : object.instances()) {
            const Polygon object_hull = gcode_print_instance_convex_hull(object, object_instance);
            for (const PrintInstance &other_instance : other->instances()) {
                const Polygon other_hull = gcode_print_instance_convex_hull(*other, other_instance);
                if (!intersection(Polygons{object_hull}, Polygons{other_hull}).empty())
                    return true;
            }
        }
    }
...
static bool gcode_uses_mixed_filament(const PrintObject &object)
{
    const Print *print = object.print();
    if (print == nullptr)
        return false;

    const std::string definitions = print->config().mixed_filament_definitions.value;
    return !boost::algorithm::trim_copy(definitions).empty();
}
...
std::vector<GCode::LayerToPrint> GCode::collect_layers_to_print(const PrintObject& object)
{
    ...
    const bool allow_alternating_initial_gap = gcode_allows_alternating_overlap_initial_gap(object);
    const bool suppress_empty_layer_warnings = allow_alternating_initial_gap || gcode_uses_mixed_filament(object);
    ...
    while (idx_object_layer < object.layers().size() || idx_support_layer < object.support_layers().size()) {
        ...
        if (layers_to_print.size() == 1u) {
            if (!has_extrusions && !suppress_empty_layer_warnings) {
                std::string warning = _(L("One object has empty initial layer and may be floating above the build plate."));
                const_cast<Print*>(object.print())
                    ->active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, warning,
    ...
    if (!warning_ranges.empty()) {
        ...
        const_cast<Print*>(object.print())
            ->active_step_add_warning(PrintStateBase::WarningLevel::NON_CRITICAL, warning, PrintStateBase::SlicingEmptyGcodeLayers);
    }
...
void GCode::_do_export(Print& print, GCodeOutputStream& file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    m_layer_count                           = 0;
    m_use_layered_for_overlapping_extruders = print.uses_layered_print_sequence_for_overlapping_extruders();
    if (print.config().print_sequence == PrintSequence::ByObject && !m_use_layered_for_overlapping_extruders) {
 ...
 */
    ...
    if (print.calib_params().mode == CalibMode::Calib_PA_Line) {
        ...
        if (print.config().print_sequence == PrintSequence::ByObject && !m_use_layered_for_overlapping_extruders && !has_wipe_tower) {
...
static std::unique_ptr<ExtrusionEntityCollection> clip_extrusion_collection_for_local_z(const ExtrusionEntityCollection& source,
                                                                                        const ExPolygons*                include_masks,
                                                                                        const ExPolygons*                exclude_masks,
                                                                                        const double flow_height_override)
{
    ...
    if ((include_masks == nullptr || include_masks->empty()) && (exclude_masks == nullptr || exclude_masks->empty()) &&
...
static void split_polyline_by_length_for_pointillism(const Polyline& src, const double split_length, Polylines& out)
{
    out.clear();
    if (!src.is_valid())
        return;
...
static bool split_extrusion_collection_for_pointillism_paths(const ExtrusionEntityCollection&                         source,
...
static bool split_extrusion_collection_for_multi_perimeter_pattern(const ExtrusionEntityCollection& source,
                                                                   ...
                                                                   size_t& out_bucket_count)
{
    ...
    for (const ExtrusionEntity* entity : flattened.entities) {
        ...
        const unsigned int extruder_id = mixed_mgr.resolve_perimeter(mixed_filament_id, num_physical, layer_index, perimeter_index);
        if (extruder_id == 0 || extruder_id > num_physical)
            continue;
                                 ...
                                 const bool prime_extruder)
{
    ...
    if (layer_tools.extruders.empty())
        // Nothing to extrude.
        return result;
    const bool is_layered_sequence    = print.config().print_sequence == PrintSequence::ByLayer || m_use_layered_for_overlapping_extruders;
    const bool is_sequential_sequence = print.config().print_sequence == PrintSequence::ByObject &&
                                        !m_use_layered_for_overlapping_extruders;
    ...
    const double pointillism_segment_len_mm     = pointillism_pixel_size_cfg > EPSILON ? std::max(0.10, pointillism_pixel_size_cfg) :
    ...
    auto configured_extruder_id = [&layer_tools](const GCode::ObjectByExtruder::Island::Region::Type entity_type,
                                                 const ExtrusionEntityCollection& entities, const PrintRegion& region) -> int {
        if (entity_type == GCode::ObjectByExtruder::Island::Region::INFILL) {
            const ExtrusionRole role = entities.entities.empty() ? erNone : entities.entities.front()->role();
            if (role == erSolidInfill && std::abs(region.config().sparse_infill_density.value - 100.) < EPSILON)
    ...
    if (local_z_perimeter_runtime_supported) {
        ...
        for (size_t layer_to_print_idx = 0; layer_to_print_idx < layers.size(); ++layer_to_print_idx) {
            ...
            auto it_interval = std::find_if(intervals.begin(), intervals.end(),
            ...
            for (size_t plan_idx = first_idx; plan_idx < end_idx; ++plan_idx) {
                ...
                const ExPolygons fixed_compensated_guard = fixed_raw_masks_union.empty() ?
                                                               ExPolygons() :
                                                               local_z_compensate_masks(fixed_raw_masks_union,
            ...
            if (ctx.pass_buckets.empty() || ctx.mixed_masks_union.empty()) {
                ...
                if (local_z_rejected_context_logs < 50) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << "Local-Z context rejected"
                                               << " print_z=" << print_z << " layer_id=" << layer_id << " first_idx=" << first_idx
                                               << " end_idx=" << end_idx << " interval_sublayer_count=" << it_interval->sublayer_count
            ...
            BOOST_LOG_TRIVIAL(debug) << "Local-Z context"
                                     << " print_z=" << print_z << " layer_id=" << layer_id << " enabled=" << ctx.enabled
    ...
    const bool local_z_perimeter_phase_b_enabled = local_z_perimeter_runtime_supported &&
    ...
    if (local_z_phase_b_requested_for_layer && !local_z_perimeter_runtime_supported) {
        BOOST_LOG_TRIVIAL(warning) << "Local-Z phase-b disabled"
                                   << " print_z=" << print_z << " wipe_tower=" << has_wipe_tower
        ...
        BOOST_LOG_TRIVIAL(warning) << "Local-Z phase-b requested but no eligible contexts"
                                   << " print_z=" << print_z << " runtime_supported=" << local_z_perimeter_runtime_supported;
    }
    ...
    for (const LayerToPrint& layer_to_print : layers) {
        ...
        if (layer_to_print.object_layer != nullptr) {
            ...
            LocalZLayerContext* local_z_ctx = (local_z_perimeter_phase_b_enabled && layer_to_print_idx < local_z_layer_contexts.size() &&
                ...
                for (const ObjectByExtruder::Island::Region::Type entity_type :
                    ...
                    for (const ExtrusionEntity* ee :
                        ...
                        if (entity_type == ObjectByExtruder::Island::Region::PERIMETERS && local_z_ctx != nullptr &&
                            !local_z_ctx->mixed_masks_union.empty()) {
                            for (LocalZPassBucket& pass_bucket : local_z_ctx->pass_buckets) {
                                ...
                                for (size_t pass_extruder_id = 0; pass_extruder_id < pass_bucket.plan->painted_masks_by_extruder.size();
                                    ...
                                    if (pass_masks.empty())
                                        continue;
                                    auto clipped_local = clip_extrusion_collection_for_local_z(*extrusions, &pass_masks, nullptr,
                                    ...
                                    if (local_height_stats.count > 0 &&
                                        ...
                                        BOOST_LOG_TRIVIAL(warning)
                                            << "Local-Z local pass height mismatch"
                                            << " print_z=" << print_z << " layer_to_print_idx=" << layer_to_print_idx
                                            ...
                                            << " observed_min=" << local_height_stats.min << " observed_max=" << local_height_stats.max
                                            << " path_count=" << local_height_stats.count;
                                    }
                                    local_z_clipped_collections.emplace_back(std::move(clipped_local));
                                    std::vector<ObjectByExtruder::Island>& islands = object_islands_by_extruder(pass_bucket.by_extruder,
                                                                                                                unsigned(pass_extruder_id),
                                                                                                                layer_to_print_idx,
                                                                                                                layers.size(),
                                                                                                                n_slices + 1);
                                    ...
                                    for (size_t i = 0; i <= n_slices; ++i) {
                                        ...
                                        if (last || entity_matches_surface(island_idx, *clipped_ptr)) {
                                            ...
                                            islands[island_idx]
                                                ...
                                                .append(ObjectByExtruder::Island::Region::PERIMETERS, clipped_ptr, nullptr);
                                            break;
                                        }
                                    }
                                }
                            ...
                            const ExPolygons* base_exclude_masks = local_z_ctx->mixed_masks_union_for_base_exclude.empty() ?
                                                                       &local_z_ctx->mixed_masks_union :
                            ...
                            const ExPolygons& mixed_leak_ref = !local_z_ctx->raw_mixed_masks_union.empty() ?
                                                                   local_z_ctx->raw_mixed_masks_union :
                                                                   local_z_ctx->mixed_masks_union;
                            ...
                            if (base_height_stats.count > 0 && local_z_ctx->base_clipped_collections <= 5) {
                                BOOST_LOG_TRIVIAL(debug)
                                    << "Local-Z base clip heights"
                                    << " print_z=" << print_z << " layer_to_print_idx=" << layer_to_print_idx
                                    << " observed_min=" << base_height_stats.min << " observed_max=" << base_height_stats.max
                        ...
                        const std::vector<unsigned int>* pointillism_sequence = is_anything_overridden ? nullptr :
                                                                                                         pointillism_sequence_for_filament(
                                                                                                             configured_filament_id);
                        if (pointillism_sequence != nullptr) {
                            ...
                            const size_t sequence_phase = pointillism_sequence->empty() ?
                                                              0 :
                                                              size_t(std::max(0, layer_tools.layer_index)) % pointillism_sequence->size();
                            if (split_extrusion_collection_for_pointillism_paths(*filtered_extrusions, *pointillism_sequence,
                                                                                 ...
                                                                                 pointillism_line_gap_scaled, sequence_phase,
                                                                                 split_by_extruder, split_stats) &&
                                ...
                                for (size_t extruder_idx = 0; extruder_idx < split_by_extruder.size(); ++extruder_idx) {
                                    ...
                                    std::vector<ObjectByExtruder::Island>& islands = object_islands_by_extruder(by_extruder,
                                                                                                                unsigned(extruder_idx),
                                                                                                                layer_to_print_idx,
                                                                                                                layers.size(),
                                                                                                                n_slices + 1);
                                    for (size_t i = 0; i <= n_slices; ++i) {
                        ...
                        if (!is_anything_overridden && entity_type == ObjectByExtruder::Island::Region::PERIMETERS &&
                            layer_tools.mixed_mgr != nullptr && layer_tools.num_physical > 0 && correct_extruder_id >= 0) {
                            const unsigned int mixed_filament_id = grouped_manual_pattern_mixed_filament_id(entity_type,
                                                                                                            *filtered_extrusions, region);
                            if (mixed_filament_id != 0) {
                                std::vector<std::unique_ptr<ExtrusionEntityCollection>> split_by_extruder;
                                size_t                                                  bucket_count = 0;
                                if (split_extrusion_collection_for_multi_perimeter_pattern(*filtered_extrusions, *layer_tools.mixed_mgr,
                                                                                           mixed_filament_id, layer_tools.num_physical,
                                                                                           layer_tools.layer_index, split_by_extruder,
                                                                                           bucket_count)) {
                                    if (bucket_count >= 2) {
                                        for (size_t extruder_idx = 0; extruder_idx < split_by_extruder.size(); ++extruder_idx) {
                                            std::unique_ptr<ExtrusionEntityCollection>& split_collection = split_by_extruder[extruder_idx];
                                            ...
                                            std::vector<ObjectByExtruder::Island>& islands =
                                                object_islands_by_extruder(by_extruder, unsigned(extruder_idx), layer_to_print_idx,
                                            ...
                                            for (size_t i = 0; i <= n_slices; ++i) {
                                                ...
                                                if (last || entity_matches_surface(island_idx, *split_ptr)) {
                                                    if (islands[island_idx].by_region.empty())
                                                        islands[island_idx].by_region.assign(print.num_print_regions(),
                                                                                             ObjectByExtruder::Island::Region());
                                                    islands[island_idx].by_region[region.print_region_id()].append(entity_type, split_ptr,
                                                                                                                   nullptr);
                                                    break;
                                                }
                                            }
                                    ...
                                    if (bucket_count == 1) {
                                        for (size_t extruder_idx = 0; extruder_idx < split_by_extruder.size(); ++extruder_idx) {
                                            const std::unique_ptr<ExtrusionEntityCollection>& split_collection =
                        ...
                        if (is_anything_overridden) {
                            entity_overrides = const_cast<LayerTools&>(layer_tools)
                                                   .wiping_extrusions()
                                                   .get_extruder_overrides(filtered_extrusions, layer_to_print.original_object,
                                                                           correct_extruder_id,
    ...
    if (local_z_perimeter_phase_b_enabled) {
        for (size_t layer_to_print_idx = 0; layer_to_print_idx < local_z_layer_contexts.size(); ++layer_to_print_idx) {
            ...
            BOOST_LOG_TRIVIAL(debug) << "Local-Z clipping summary"
                                     ...
                                     << " base_collections=" << ctx.base_clipped_collections << " pass_count=" << ctx.pass_buckets.size();
        }
    }
    ...
    if (local_z_perimeter_phase_b_enabled) {
        BOOST_LOG_TRIVIAL(info) << "Local-Z phase-b prepared"
                                << " print_z=" << print_z << " perimeter_passes=" << local_z_pass_refs.size();
    ...
    if (!local_z_pass_refs.empty()) {
        ...
        auto emit_local_z_pass_for_extruder = [&](const LocalZPassRef& pass_ref, unsigned int local_extruder_id) {
            ...
            BOOST_LOG_TRIVIAL(debug) << "Local-Z pass emit"
                                     << " print_z=" << print_z << " layer_to_print_idx=" << pass_ref.layer_to_print_idx
                                     << " layer_id=" << pass_plan.layer_id << " pass_index=" << pass_plan.pass_index
                                     << " dependency_group=" << pass_plan.dependency_group
                                     << " dependency_order=" << pass_plan.dependency_order << " pass_print_z=" << pass_plan.print_z
                                     << " pass_flow_height=" << pass_plan.flow_height << " extruder=" << local_extruder_id;
            ...
            if (std::abs(m_writer.get_position().z() - pass_z) > EPSILON) {
                BOOST_LOG_TRIVIAL(warning) << "Local-Z pass z restore"
                                           << " print_z=" << print_z << " layer_id=" << pass_plan.layer_id
                                           << " pass_index=" << pass_plan.pass_index << " dependency_group=" << pass_plan.dependency_group
                                           << " dependency_order=" << pass_plan.dependency_order << " extruder=" << local_extruder_id
            ...
            std::vector<InstanceToPrint> instances_to_print = sort_print_object_instances(objects_by_extruder, layers, ordering,
                ...
                if (pass_group_end < local_z_pass_refs.size()) {
                    ...
                    for (size_t group_idx = pass_group_end; group_idx < next_group_end; ++group_idx) {
                        for (unsigned int extruder_id : local_z_bucket_extruders(*local_z_pass_refs[group_idx].bucket))
                            if (std::find(next_group_extruders.begin(), next_group_extruders.end(), extruder_id) ==
                                next_group_extruders.end())
        ...
        const bool dependency_chain_mode = std::all_of(local_z_pass_refs.begin(), local_z_pass_refs.end(),
                                                       [](const LocalZPassRef& pass_ref) {
                                                           return pass_ref.bucket != nullptr && pass_ref.bucket->plan != nullptr &&
            ...
            while (dependency_scheduler_ok && completed_passes < pass_states.size()) {
                ...
                if (chosen_extruder < 0) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << "Local-Z dependency scheduler deadlocked, falling back"
                                               << " print_z=" << print_z << " pass_count=" << local_z_pass_refs.size();
                    break;
                }
                ...
                if (ready_pass_indices.empty()) {
                    ...
                    BOOST_LOG_TRIVIAL(warning) << "Local-Z dependency scheduler made no progress, falling back"
                                               << " print_z=" << print_z << " chosen_extruder=" << chosen_extruder
    ...
    for (unsigned int extruder_id : layer_extruders) {
        ...
        bool                         has_prime_tower = print.config().enable_prime_tower && print.extruders().size() > 1 &&
                                                       ((is_layered_sequence && print.config().print_order == PrintOrder::Default) ||
                                                        (is_sequential_sequence && print.objects().size() == 1));
        if (has_prime_tower) {
        ...
        // BBS
        if (print.config().skirt_type == stPerObject && is_sequential_sequence && !layer.object()->object_skirt().empty() &&
    ...
    if (pointillism_path_split_entities > 0) {
        BOOST_LOG_TRIVIAL(warning) << "Same-layer pointillisme path-domain split"
                                   << " layer_id=" << layer.id() << " print_z=" << print_z
                                   << " entities=" << pointillism_path_split_entities << " segments=" << pointillism_path_split_segments
                                   << " segment_len_mm=" << pointillism_segment_len_mm << " line_gap_mm=" << pointillism_line_gap_mm
...
std::string GCode::_extrude(const ExtrusionPath& path, std::string description, double speed)
{
    ...
    const double pointillism_nominal_segment_mm =
        pointillism_pixel_size_mm > EPSILON ?
    ...
    const bool   skip_accel_jerk_switch_for_short_pointillism = pointillism_path &&
                                                                path_length_mm <= pointillism_min_accel_switch_len_mm + EPSILON;
